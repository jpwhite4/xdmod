...
class ETLOverseerBootStrap
{
    ...
    static public function execute() {
        ...
        $args = Console_Getopt::getopt($args, implode('', array_keys($options)), array_values($options));
        ...
        foreach ($args as $arg => $value) {
            ...
            case 'base-dir':
                if ( ! is_dir($value) ) {
                    return self::usage_and_exit("Base directory does not exist: '$value'");
                }
            ...
            case 'chunk-size-days':
                ...
                case 'year':
                    ...
                    break;
                default:
                    return self::usage_and_exit("Invalid chunk size: $value");
                    break;
                }
                break;
                ...
                case 'end-date':
                    if ( false === strtotime($value) ) {
                        return self::usage_and_exit("Could not parse end date: '$value'");
                    }
                ...
                case 'last-modified-start-date':
                    if ( false === strtotime($value) ) {
                        return self::usage_and_exit("Could not parse last modified start date: '$value'");
                    }
                ...
                case 'number-of-days':
                    ...
                    if ($scriptOptions['number-of-days'] < 1) {
                        return self::usage_and_exit("$arg must be an integer greater than 0");
                    }
                    break;
                ...
                case 'option':
                    ...
                    foreach ( $value as $option ) {
                        ...
                        if ( 2 != count($parts) ) {
                            return self::usage_and_exit("Options must be of the form option=value: '$option'");
                        }
                ...
                case 'start-date':
                    if ( false === strtotime($value) ) {
                        return self::usage_and_exit("Could not parse start date: '$value'");
                    }
                ...
                case 'verbosity':
                    ...
                    case 'quiet':
                        ...
                        break;
                    default:
                        return self::usage_and_exit("Invalid verbosity level: $value");
                        break;
                    }  // switch ( $value )
                    break;
                    ...
                    case 'last-modified-end-date':
                        if ( false === strtotime($value) ) {
                            return self::usage_and_exit("Could not parse last modified end date: '$value'");
                        }
                    ...
                    case 'h':
                    case 'help':
                        return self::usage_and_exit();
                        break;

                    default:
                        return self::usage_and_exit("Invalid option: $arg");
                        break;
            }
        }  // foreach ($args as $arg => $value)
        ...
        foreach ( $argv as $index => $arg ) {
            ...
            if ( null !== $opt && ! in_array($opt, $parsedArgs) ) {
                return self::usage_and_exit("Unparsed argument: $arg");
            }
        }
        ...
        if ( null === $scriptOptions['config-file'] ) {
            return self::usage_and_exit("Config file required");
        } elseif ( null !== $scriptOptions['config-file'] && ! is_file($scriptOptions['config-file']) ) {
            return self::usage_and_exit("Config file not found: '" . $scriptOptions['config-file'] . "'");
        }
        ...
        try {
            ...
            return self::log_error_and_exit(
        ...
        if ( false === ($utilityEndpoint = $etlConfig->getGlobalEndpoint('utility')) ) {
            return self::log_error_and_exit(sprintf(
                "%s%s%s",
        ...
        if ( $showList ) {
            foreach ( $listOptions as $opt ) {
                ...
                case 'list-resources':
                    ...
                    print implode(self::LIST_SEPARATOR, $headings) . "\n";

                    foreach ( $result as $row ) {
                        $fields = array($row['code'], $row['start_date'], $row['end_date']);
                        print implode(self::LIST_SEPARATOR, $fields) . "\n";
                    }
                    break;
                ...
                case 'list-configured-endpoints':
                    ...
                    for ($i=0; $i < count($endpointSummary[0]); $i++) {
                        ...
                        print implode(self::LIST_SEPARATOR, $a) . "\n";
                    }
                    break;
    ...
    static private function log_error_and_exit($msg)
    {
        global $logger;
        $logger->err($msg);
    ...
    static private function usage_and_exit($msg = null)
    {
